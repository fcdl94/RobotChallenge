...
from CombinedNet.custom_layers import CombinedLayers
import math
...
class BasicMaskedBlock(nn.Module):  # Define a residual block
    ...
    def __init__(self, inplanes, planes, stride=1, classes=1, order=None):
        super(BasicMaskedBlock, self).__init__()

        self.classes = classes
        self.index = 0

        convBlock = CombinedLayers

        self.conv1 = convBlock(inplanes, planes, kernel_size=3, stride=stride, padding=1, mask=classes, order=order)
        self.bn1 = nn.ModuleList([nn.BatchNorm2d(planes) for i in range(classes)])
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = convBlock(planes, planes, kernel_size=3, padding=1, mask=classes, order=order)
        self.bn2 = nn.ModuleList([nn.BatchNorm2d(planes) for i in range(classes)])

        self.first = inplanes != planes
        if inplanes != planes:
            self.downsample = nn.Sequential(
                convBlock(inplanes, planes, kernel_size=1, stride=stride, mask=classes, order=order)
            )
...
class MaskedNet(nn.Module):
    def __init__(self, layers, classes=[1000], fc=True, order=None):
        super(MaskedNet, self).__init__()

        convBlock = CombinedLayers

        if order is None:
            order = [i for i in range(len(classes))]
        ...
        self.conv1 = convBlock(3, 64, kernel_size=7, stride=2, padding=3, mask=self.models, order=order)
    ...
    def set_index(self, index):
        if 0 <= index < self.models:
            self.index = index
            for m in self.modules():
                if isinstance(m, CombinedLayers) or isinstance(m, BasicMaskedBlock):
                    m.set_index(index)
...
def combined_net18(model_classes, pre_imagenet=True, pretrained=None, fc=True, order=None):
    if order is None:
        order = range(model_classes)
    model = MaskedNet(classes=model_classes, layers=[2, 2, 2, 2], fc=fc, order=order)
